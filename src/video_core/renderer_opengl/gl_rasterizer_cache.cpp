...
VAddr ToCpuAddr(GPUVAddr addr) {
    return *Core::System::GetInstance().GPU().memory_manager->GpuToCpuAddress(addr);
}
...
void MortonCopy(u32 stride, u32 block_height, u32 height, u8* gl_buffer, GPUVAddr base,
                GPUVAddr start, GPUVAddr end) {
    constexpr u32 bytes_per_pixel = SurfaceParams::GetFormatBpp(format) / 8;
    constexpr u32 gl_bytes_per_pixel = CachedSurface::GetGLBytesPerPixel(format);

    if (morton_to_gl) {
        auto data = Tegra::Texture::UnswizzleTexture(
            ToCpuAddr(base), SurfaceParams::TextureFormatFromPixelFormat(format), stride, height,
            block_height);
        ...
        VideoCore::MortonCopyPixels128(stride, height, bytes_per_pixel, gl_bytes_per_pixel,
                                       Memory::GetPointer(ToCpuAddr(base)), gl_buffer,
                                       morton_to_gl);
    }
}

static constexpr std::array<void (*)(u32, u32, u32, u8*, GPUVAddr, GPUVAddr, GPUVAddr),
...
SurfaceParams SurfaceParams::FromInterval(SurfaceInterval interval) const {
    ...
    GPUVAddr aligned_start =
        addr + Common::AlignDown(boost::icl::first(interval) - addr, stride_tiled_bytes);
    GPUVAddr aligned_end =
...
void CachedSurface::LoadGLBuffer(GPUVAddr load_start, GPUVAddr load_end) {
...
void CachedSurface::FlushGLBuffer(GPUVAddr flush_start, GPUVAddr flush_end) {
...
boost::optional<GPUVAddr> RasterizerCacheOpenGL::TryFindSurfaceAddress(VAddr cpu_addr) {
    std::vector<GPUVAddr> gpu_addresses;
    for (const auto& pair : surface_cache) {
        for (const auto& surface : pair.second) {
            if (cpu_addr == surface->CpuStartAddr()) {
    ...
    ASSERT(gpu_addresses.size() == 1);
    return gpu_addresses[0];
}

SurfaceRect_Tuple RasterizerCacheOpenGL::GetSurfaceSubRect(SurfaceParams& params,
...
void RasterizerCacheOpenGL::ValidateSurface(const Surface& surface, GPUVAddr addr, u64 size) {
    if (size == 0)
        return;
...
void RasterizerCacheOpenGL::FlushRegion(GPUVAddr addr, u64 size, Surface flush_surface) {
    if (size == 0)
        return;
...
void RasterizerCacheOpenGL::InvalidateRegion(GPUVAddr addr, u64 size, const Surface& region_owner) {
    if (size == 0)
        return;
...
void RasterizerCacheOpenGL::UpdatePagesCachedCount(GPUVAddr addr, u64 size, int delta) {
    const u64 num_pages =
    ...
    for (const auto& pair : RangeFromInterval(cached_pages, pages_interval)) {
        ...
        const GPUVAddr interval_start_addr = boost::icl::first(interval) << Memory::PAGE_BITS;
        const GPUVAddr interval_end_addr = boost::icl::last_next(interval) << Memory::PAGE_BITS;
