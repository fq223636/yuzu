...
void Linker::FindExternals(std::vector<u8>& program_image, const std::vector<Symbol>& symbols,
        ...
        case RelocationType::GLOB_DAT:
            if (!symbol.value) {
                ...
                if (symbol.name == "memcpy") {
                    Core::System::GetInstance().ArmInterface(0).WriteHleHook(rela.offset +
                                                                             load_base);
                    Core::System::GetInstance().ArmInterface(1).WriteHleHook(rela.offset +
                                                                             load_base);
                    Core::System::GetInstance().ArmInterface(2).WriteHleHook(rela.offset +
                                                                             load_base);
                    Core::System::GetInstance().ArmInterface(3).WriteHleHook(rela.offset +
                                                                             load_base);
                }
            } else {
...
void Linker::ParseDynamicSection(std::vector<u8>& program_image, u32 dynamic_section_offset,
                                 VAddr load_base) {
    ...
    if (dynamic.find(DT_RELA) != dynamic.end()) {
        FindExternals(program_image, symbols, dynamic[DT_RELA], dynamic[DT_RELASZ], load_base);
    }

    if (dynamic.find(DT_JMPREL) != dynamic.end()) {
        FindExternals(program_image, symbols, dynamic[DT_JMPREL], dynamic[DT_PLTRELSZ], load_base);
    }
}
