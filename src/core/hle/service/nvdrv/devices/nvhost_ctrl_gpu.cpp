...
u32 nvhost_ctrl_gpu::ioctl(u32 command, const std::vector<u8>& input, std::vector<u8>& output) {
    ...
    case IocZcullGetCtxSizeCommand:
        return ZCullGetCtxSize(input, output);
    case IocZcullGetInfo:
...
u32 nvhost_ctrl_gpu::GetTPCMasks(const std::vector<u8>& input, std::vector<u8>& output) {
    ...
    LOG_WARNING(Service_NVDRV, "(STUBBED) called, mask=0x%x, mask_buf_addr=0x%lx", params.mask_buf_size,
...
u32 nvhost_ctrl_gpu::ZCullGetCtxSize(const std::vector<u8>& input, std::vector<u8>& output) {
    ...
    std::memcpy(&params, input.data(), input.size());
    params.size = 0x0; // Got 0x00000000 on hardware
    std::memcpy(output.data(), &params, output.size());
    return 0;
}
